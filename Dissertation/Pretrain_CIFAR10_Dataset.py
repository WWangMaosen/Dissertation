...
def train(args, model, device, train_loader, optimizer, epoch, num_classes):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if epoch<300:
            ...
            nc = torch.amax(torch.absolute(fc1grad))  # æœ€å¤§å€¼
            fc1grad = fc1grad / nc
        ...
        if epoch % 20 == 0:
            torch.save(fc1, '2000_CF_Neurons.pt')
...
def main():
    ...
    parser.add_argument('--test-batch-size', type=int, default=450, metavar='N',
                        help='input batch size for testing (default: 1000)')
    parser.add_argument('--epochs', type=int, default=300, metavar='N', help='number of epochs to train (default: 14)')
    parser.add_argument('--lr', type=float, default=5, metavar='LR', help='learning rate (default: 1.0)')
    parser.add_argument('--gamma', type=float, default=0.95, metavar='M',
                        help='Learning rate step gamma (default: 0.7)')
    parser.add_argument('--step-size', type=float, default=1, metavar='M', help='Scheduler step size (default: 10)')
    ...
    train_dataset = datasets.CIFAR10('/manas/data/', train=True, transform=train_transform, download=True)
    val_dataset = datasets.CIFAR10('/manas/data/', train=False, transform=test_transform, download=True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=16,
                                               pin_memory=True)
    test_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=16,
                                              pin_memory=True)

    # Initialize weights
    fc1 = torch.normal(-0.5,1.0,(3072,2000),device=device, requires_grad=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 2 == 0:
            ...
            fc1_weights = model[0].data.cpu().numpy()
